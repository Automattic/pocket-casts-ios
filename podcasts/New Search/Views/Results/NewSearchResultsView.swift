...
struct NewSearchResultsView: View {
    ...
    @ViewBuilder var filterPicker: some View {
        ...
        .padding(.bottom, 8)
        .background(LiquidGlass.isEnabled ? theme.primaryUi02 : theme.secondaryUi01)
    ...
    @ViewBuilder var localResults: some View {
        ForEach(searchResults.podcasts, id: \.self) { localPodcast in
            SearchResultCell(episode: nil, result: localPodcast, played: false, showDivider: false)
                .listRowBackground(theme.primaryUi02)
                .alignmentGuide(.listRowSeparatorLeading) { _ in
                    return 0
                }
    ...
    @ViewBuilder var predictiveList: some View {
        ForEach(searchResults.predictive.prefix(Constants.maxNumberOfEpisodes), id: \.self) { predictiveSearch in
            switch predictiveSearch.type {
                ...
                case .podcast:
                    SearchResultCell(episode: nil, result: PodcastFolderSearchResult(from: predictiveSearch), played: false, showDivider: false)
                        .listRowBackground(theme.primaryUi02)
                        .alignmentGuide(.listRowSeparatorLeading) { _ in
                            return 0
                        }
