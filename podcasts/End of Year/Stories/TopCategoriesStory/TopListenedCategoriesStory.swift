...
struct TopListenedCategoriesStory: ShareableStory {
    ...
    func pillar(_ index: Int) -> some View {
        if let listenedCategory = listenedCategories[safe: index] {
            CategoryPillar(color: contrastColor.tintColor, text: "\(index + 1)", title: listenedCategory.categoryTitle.localized, subtitle: listenedCategory.totalPlayedTime.storyTimeDescription, height: CGFloat(200 - (index * 55)))
