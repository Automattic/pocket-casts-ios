...
class SyncHistoryTask: ApiBaseTask {
    ...
    override func apiTokenAcquired(token: String) {
        ...
        if let clearDate = Settings.lastClearHistoryDate() {
        ...
        let url = Server.Urls.api + "history/sync"
        do {
    ...
    private func process(serverData: Data) {
        do {
            ...
            UserDefaults.standard.set("\(response.serverModified)", forKey: Constants.UserDefaults.historyServerLastModified)
    ...
    private func updateEpisodes(updates: [Api_HistoryChange]) {
        for (index, change) in updates.enumerated() {
            ...
            if index > Constants.Limits.maxHistoryItems { return }
    ...
    private func historyServerModified() -> Int64? {
        if let modifiedStr = UserDefaults.standard.string(forKey: Constants.UserDefaults.historyServerLastModified), modifiedStr.count > 0 {
