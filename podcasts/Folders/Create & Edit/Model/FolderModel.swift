...
class FolderModel: ObservableObject {
    ...
    @Published var selectedPodcastUuids: [String] = [] {
        didSet {
            ...
            DataManager.shared.bulkSetFolderUuid(folderUuid: folderUuid, podcastUuids: selectedPodcastUuids)
    ...
    @Published var name: String = "" {
        didSet {
            ...
            if let folder = DataManager.shared.findFolder(uuid: folderUuid) {
    ...
    @Published var colorInt: Int = 0 {
        didSet {
            ...
            DataManager.shared.updateFolderColor(folderUuid: folderUuid, color: Int32(colorInt), syncModified: TimeFormatter.currentUTCTimeInMillis())
            NotificationCenter.postOnMainThread(notification: Constants.Notifications.folderChanged, object: folderUuid)
    ...
    func createFolder() -> String {
        ...
        DataManager.shared.bulkSetFolderUuid(folderUuid: folder.uuid, podcastUuids: selectedPodcastUuids)
    ...
    func deleteFolder() {
        ...
        DataManager.shared.delete(folderUuid: folderUuid, markAsDeleted: SyncManager.isUserLoggedIn())
    ...
    private func updateFoldersBasedOnSelection() {
        ...
        for uuid in selectedPodcastUuids {
            guard let podcast = DataManager.shared.findPodcast(uuid: uuid), let previousFolderUuid = podcast.folderUuid else { continue }

            foldersChanged.insert(previousFolderUuid)
        }

        DataManager.shared.bulkSetSyncModified(TimeFormatter.currentUTCTimeInMillis(), onFolders: Array(foldersChanged))
    }
}
