...
class BadgeHelper {
    ...
    private func badgeCount() -> Int? {
        ...
        case .off:
            return 0
        case .totalUnplayed:
            return DataManager.shared.count(query: "SELECT COUNT(e.id) FROM SJEpisode e LEFT JOIN SJPodcast p ON p.id = e.podcast_id WHERE p.subscribed = 1 AND e.playingStatus == 1 AND e.archived = 0", values: nil)
        case .newSinceLastOpened:
            ...
            return DataManager.shared.count(query: "SELECT COUNT(e.id) FROM SJEpisode e LEFT JOIN SJPodcast p ON p.id = e.podcast_id WHERE p.subscribed = 1 AND e.playingStatus == 1 AND e.archived = 0 AND e.addedDate > ?", values: [lastClosedDate])
        case .filterCount:
            guard let playlistId = Settings.appBadgeFilterUuid,
                  let playlist = DataManager.shared.findPlaylist(uuid: playlistId) else {
