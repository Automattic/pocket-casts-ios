...
private struct YouTubeHubSubscriptionsView: View {
    ...
    private var subscriptionsList: some View {
        List(viewModel.subscriptions) { subscription in
            NavigationLink {
                YouTubeFeedDetailView(feed: subscription.toFeed())
                    .environmentObject(theme)
            } label: {
...
private struct YouTubeHubSubscriptionRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            // Add to feeds button (stops propagation to NavigationLink)
            if isAddedToFeeds {
    ...
    private func addToFeeds() {
        YouTubeFeedManager.shared.addFeed(subscription.toFeed())
