...
public struct GRDBRecordMacro: MemberMacro, ExtensionMacro {
    ...
    public static func expansion(
        ...
        if isClass {
            ...
            members.append(generateDecodableInit(properties: properties))
    ...
    private static func generateColumns(properties: [PropertyInfo]) -> DeclSyntax {
        let columns = properties.map { "public static let \($0.name) = Column(CodingKeys.\($0.name))" }.joined(separator: "\n        ")
