...
final class UpNextDataManagerTests: DataManagerTestCase {
    ...
    func testAllUpNextPlaylistEpisodesReturnsEpisodes() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(playlistEpisodes.count, 2, "Should return 2 playlist episodes")
        }
    }
    ...
    func testAllUpNextPlaylistEpisodesReturnsEmptyWhenNone() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertTrue(playlistEpisodes.isEmpty, "Should return empty when no episodes in Up Next")
        }
    }
    ...
    func testUpNextPlayListContainsReturnsTrueWhenContains() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertTrue(contains, "Should contain episode")
        }
    }
    ...
    func testUpNextPlayListContainsReturnsFalseWhenNotContains() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertFalse(contains, "Should not contain non-existent episode")
        }
    }
    ...
    func testAllUpNextEpisodesReturnsBaseEpisodes() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(upNextEpisodes.count, 2, "Should return 2 episodes")
        }
    }
    ...
    func testDeleteAllUpNextEpisodesRemovesAll() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.allUpNextPlaylistEpisodes().count, 3, "Should have 3 episodes initially")

            dataManager.deleteAllUpNextEpisodes()

            XCTAssertEqual(dataManager.allUpNextPlaylistEpisodes().count, 0, "Should have 0 episodes after deletion")
        }
    }
    ...
    func testDeleteAllUpNextEpisodesOnEmptyDoesNotCrash() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.allUpNextPlaylistEpisodes().count, 0, "Should not crash on empty Up Next")
        }
    }
    ...
    func testDeleteAllUpNextEpisodesExceptKeepsOneEpisode() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(remaining.count, 1, "Should keep only one episode")
            XCTAssertEqual(remaining.first?.episodeUuid, "episode-to-keep", "Should keep correct episode")
        }
    }
    ...
    func testDeleteAllUpNextEpisodesExceptWithNonExistentDeletesAll() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.allUpNextPlaylistEpisodes().count, 0, "Should delete all when exception episode doesn't exist")
        }
    }
    ...
    func testDeleteAllUpNextEpisodesNotInKeepsMatchingEpisodes() throws {
        try runWithDataManager { dataManager in
            ...
            let remaining = dataManager.allUpNextPlaylistEpisodes()
            XCTAssertEqual(remaining.count, 2, "Should keep 2 episodes")
            let uuids = remaining.map { $0.episodeUuid }
            XCTAssertTrue(uuids.contains("keep-1"), "Should contain keep-1")
            XCTAssertTrue(uuids.contains("keep-2"), "Should contain keep-2")
        }
    }
    ...
    func testDeleteAllUpNextEpisodesNotInWithEmptyArrayDeletesAll() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.allUpNextPlaylistEpisodes().count, 0, "Should delete all when empty array")
        }
    }
    ...
    func testDeleteAllUpNextEpisodesInDeletesMatchingEpisodes() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(remaining.count, 2, "Should keep 2 episodes")
            let uuids = remaining.map { $0.episodeUuid }
            XCTAssertFalse(uuids.contains("delete-1"), "Should not contain delete-1")
            XCTAssertFalse(uuids.contains("delete-2"), "Should not contain delete-2")
        }
    }
    ...
    func testDeleteAllUpNextEpisodesInWithEmptyArrayDoesNothing() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.allUpNextPlaylistEpisodes().count, 2, "Should keep all when empty array")
        }
    }
    ...
    func testDeleteAllUpNextEpisodesInWithNonExistentDoesNothing() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(dataManager.allUpNextPlaylistEpisodes().count, 2, "Should keep all when UUIDs don't exist")
        }
    }
    ...
    func testSaveUpNextAddToTopAddsToTop() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(episodes.count, 2, "Should have 2 episodes")
        }
    }
    ...
    func testSaveUpNextRemoveRemovesEpisode() throws {
        try runWithDataManager { dataManager in
            ...
            self.addToUpNextBottom(episodeUuid: episode.uuid, title: episode.title ?? "", podcastUuid: podcast.uuid, dataManager: dataManager)
            XCTAssertTrue(dataManager.upNextPlayListContains(episodeUuid: episode.uuid), "Should contain episode initially")
            ...
            XCTAssertFalse(dataManager.upNextPlayListContains(episodeUuid: episode.uuid), "Should not contain episode after removal")
        }
    }
}
