...
final class FolderDataManagerTests: DataManagerTestCase {
    ...
    func testFindFolderReturnsFolder() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertNotNil(found, "Should find folder")
            XCTAssertEqual(found?.uuid, folder.uuid, "UUID should match")
            XCTAssertEqual(found?.name, folder.name, "Name should match")
        }
    }
    ...
    func testFindFolderReturnsNilForNonExistent() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertNil(found, "Should not find non-existent folder")
        }
    }
    ...
    func testAllFoldersReturnsAllFolders() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(folders.count, 3, "Should return all 3 folders")
        }
    }
    ...
    func testAllFoldersExcludesDeletedByDefault() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(folders.count, 1, "Should exclude deleted folder")
            XCTAssertEqual(folders.first?.name, "Active", "Should return active folder")
        }
    }
    ...
    func testAllFoldersIncludesDeletedWhenRequested() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(folders.count, 2, "Should include deleted folder")
        }
    }
    ...
    func testDeleteFolderRemovesFolder() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertNil(found, "Should delete folder")
        }
    }
    ...
    func testDeleteFolderMarkAsDeletedKeepsFolder() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertNotNil(found, "Folder should still exist")
            XCTAssertTrue(found?.wasDeleted == true, "Folder should be marked as deleted")
        }
    }
    ...
    func testUpdateFolderColorUpdatesColor() throws {
        try runWithDataManager { dataManager in
            ...
            let found = dataManager.findFolder(uuid: folder.uuid)
            XCTAssertEqual(found?.color, 5, "Color should be updated")
            XCTAssertEqual(found?.syncModified, 12345, "syncModified should be updated")
        }
    }
    ...
    func testUpdateFolderSyncModifiedUpdatesSyncModified() throws {
        try runWithDataManager { dataManager in
            ...
            let found = dataManager.findFolder(uuid: folder.uuid)
            XCTAssertEqual(found?.syncModified, 99999, "syncModified should be updated")
        }
    }
    ...
    func testBulkSetSyncModifiedUpdatesMultipleFolders() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(updated1?.syncModified, 54321, "Folder 1 syncModified should be updated")
            XCTAssertEqual(updated2?.syncModified, 54321, "Folder 2 syncModified should be updated")
            XCTAssertEqual(unchanged?.syncModified, 0, "Folder 3 syncModified should be unchanged")
        }
    }
    ...
    func testBulkSetSyncModifiedWithEmptyArrayDoesNothing() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(unchanged?.syncModified, 100, "syncModified should be unchanged")
        }
    }
    ...
    func testMarkAllFoldersSyncedSetsAllToZero() throws {
        try runWithDataManager { dataManager in
            ...
            let folders = dataManager.allFolders(includeDeleted: true)
            XCTAssertTrue(folders.allSatisfy { $0.syncModified == 0 }, "All folders should have syncModified = 0")
        }
    }
    ...
    func testAllUnsyncedFoldersReturnsUnsyncedFolders() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(unsynced.count, 2, "Should return 2 unsynced folders")
            XCTAssertTrue(unsynced.allSatisfy { $0.syncModified != 0 }, "All should have non-zero syncModified")
        }
    }
    ...
    func testSaveSortOrdersUpdatesSortOrders() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(updated1?.sortOrder, 3, "Folder 1 sortOrder should be 3")
            XCTAssertEqual(updated2?.sortOrder, 1, "Folder 2 sortOrder should be 1")
            XCTAssertEqual(updated3?.sortOrder, 2, "Folder 3 sortOrder should be 2")
            XCTAssertEqual(updated1?.syncModified, 33333, "syncModified should be updated")
        }
    }
    ...
    func testSaveSortOrdersWithEmptyArrayDoesNothing() throws {
        try runWithDataManager { dataManager in
            ...
            let unchanged = dataManager.findFolder(uuid: folder.uuid)
            XCTAssertEqual(unchanged?.sortOrder, 5, "sortOrder should be unchanged")
            XCTAssertEqual(unchanged?.syncModified, 0, "syncModified should be unchanged")
        }
    }
    ...
    func testClearAllFolderInformationRemovesAllFolders() throws {
        try runWithDataManager { dataManager in
            ...
            let folders = dataManager.allFolders(includeDeleted: true)
            XCTAssertEqual(folders.count, 0, "Should remove all folders")
        }
    }
    ...
    func testClearAllFolderInformationOnEmptyTableDoesNotCrash() throws {
        try runWithDataManager { dataManager in
            ...
            let folders = dataManager.allFolders(includeDeleted: true)
            XCTAssertEqual(folders.count, 0, "Should not crash on empty table")
        }
    }
    ...
    func testDeleteAllFoldersAndMarkSyncMarksAllAsDeleted() throws {
        try runWithDataManager { dataManager in
            ...
            let folders = dataManager.allFolders(includeDeleted: true)
            XCTAssertTrue(folders.allSatisfy { $0.wasDeleted == true }, "All folders should be marked as deleted")
        }
    }
    ...
    func testSaveInsertsNewFolderWithAllFields() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertNotNil(found, "Should find saved folder")
            XCTAssertEqual(found?.uuid, "save-test-uuid", "UUID should match")
            XCTAssertEqual(found?.name, "Save Test Folder", "Name should match")
            XCTAssertEqual(found?.color, 5, "Color should match")
            XCTAssertEqual(found?.sortOrder, 10, "Sort order should match")
            XCTAssertEqual(found?.sortType, 2, "Sort type should match")
            XCTAssertEqual(found?.wasDeleted, false, "wasDeleted should match")
            XCTAssertEqual(found?.syncModified, 12345, "syncModified should match")
        }
    }
    ...
    func testSaveUpdatesExistingFolder() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertEqual(found?.color, 9, "Color should be updated")
            XCTAssertEqual(found?.sortOrder, 99, "Sort order should be updated")
            XCTAssertEqual(found?.syncModified, 99999, "syncModified should be updated")
        }
    }
    ...
    func testSaveGeneratesUuidIfEmpty() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertFalse(folder.uuid.isEmpty, "UUID should be generated")

            let found = dataManager.findFolder(uuid: folder.uuid)
            XCTAssertNotNil(found, "Should find folder with generated UUID")
        }
    }
    ...
    func testSavePreservesAddedDate() throws {
        try runWithDataManager { dataManager in
            ...
            XCTAssertNotNil(found?.addedDate, "addedDate should be set")
            // Compare timestamps (allow small variance for floating point)
            if let foundDate = found?.addedDate {
                XCTAssertEqual(foundDate.timeIntervalSince1970, originalDate.timeIntervalSince1970, accuracy: 1.0, "addedDate should match")
            }
        }
    }
